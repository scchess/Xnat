...
function confirmValues(_focus){
  ...
  try{
	  ...
	  if(month.value!="" && month.value!="bad"){
	     removeAppendedIcon("xnat:petmrSessionData.date.month");
	  }else{
	   	  appendIcon("xnat:petmrSessionData.date.month","fa-asterisk","Required",{ color: '#c66' });
	   	  valid=false;
	  }
	  	  
	  if(day.value!="" && day.value!="bad"){
	     removeAppendedIcon("xnat:petmrSessionData.date.date");
	  }else{
	   	  appendIcon("xnat:petmrSessionData.date.date","fa-asterisk","Required",{ color: '#c66' });
	   	  valid=false;
	  }
	  	  
	  if(year.value!="" && year.value!="bad"){
	     removeAppendedIcon("xnat:petmrSessionData.date.year");
	  }else{
	   	  appendIcon("xnat:petmrSessionData.date.year","fa-asterisk","Required",{ color: '#c66' });
	   	  valid=false;
	  }
	  ...
	  if(tracer.value!="" && tracer.value!="bad"){
	    if(document.getElementById("SEL_xnat:petmrSessionData.tracer.name")!=undefined){
	  		removeAppendedIcon("SEL_xnat:petmrSessionData.tracer.name");
	  	}else{
	  		removeAppendedIcon("xnat:petmrSessionData.tracer.name");
	  	}
	  }else{
	  	if(document.getElementById("SEL_xnat:petmrSessionData.tracer.name")!=undefined){
	  		appendIcon("SEL_xnat:petmrSessionData.tracer.name","fa-asterisk","Required",{ color: '#c66' });
	  	}else{
	  		appendIcon("xnat:petmrSessionData.tracer.name","fa-asterisk","Required",{ color: '#c66' });
	  	}
	   	  valid=false;
	  }
			...
			var em=getValueById("xnat:petmrSessionData/start_time_scan.seconds");
		  	...
		  	if(ih.value!="" && ih.value!="bad"){
		  		if(im.value!="" && im.value!="bad"){
				    removeAppendedIcon("xnat:petmrSessionData/start_time_injection.minutes");
				}else{
				   	appendIcon("xnat:petmrSessionData/start_time_injection.minutes","fa-asterisk","Required",{ color: '#c66' });
				   	valid=false;
				}
		  	}
		  	...
		  	if(eh.value!="" && eh.value!="bad"){
		  		if(em.value!="" && em.value!="bad"){
				    removeAppendedIcon("xnat:petmrSessionData/start_time_scan.minutes");
				}else{
				   	appendIcon("xnat:petmrSessionData/start_time_scan.minutes","fa-asterisk","Required",{ color: '#c66' });
				   	valid=false;
				}
		  	}
