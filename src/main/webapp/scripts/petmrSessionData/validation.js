...
/*
 * D:/Development/XNAT/1.6/xnat_builder_1_6dev/plugin-resources/webapp/xnat/scripts/petmrSessionData/validation.js
...
function confirmValues(_focus){
  ...
  try{
	  ...
	  var month=getValueById("xnat:petmrSessionData.date.month");
	  var year=getValueById("xnat:petmrSessionData.date.year");
	  var day=getValueById("xnat:petmrSessionData.date.date");
	  	  
	  if(month.value!="" && month.value!="bad"){
	     removeAppendImage("xnat:petmrSessionData.date.month");
	  }else{
	   	  appendImage("xnat:petmrSessionData.date.month","/images/checkmarkRed.gif");
	   	  valid=false;
	  }
	  	  
	  if(day.value!="" && day.value!="bad"){
	     removeAppendImage("xnat:petmrSessionData.date.date");
	  }else{
	   	  appendImage("xnat:petmrSessionData.date.date","/images/checkmarkRed.gif");
	   	  valid=false;
	  }
	  	  
	  if(year.value!="" && year.value!="bad"){
	     removeAppendImage("xnat:petmrSessionData.date.year");
	  }else{
	   	  appendImage("xnat:petmrSessionData.date.year","/images/checkmarkRed.gif");
	   	  valid=false;
	  }
  
	  var tracer=getValueById("xnat:petmrSessionData.tracer.name");
	  if(tracer.value!="" && tracer.value!="bad"){
	    if(document.getElementById("SEL_xnat:petmrSessionData.tracer.name")!=undefined){
	  		removeAppendImage("SEL_xnat:petmrSessionData.tracer.name");
	  	}else{
	  		removeAppendImage("xnat:petmrSessionData.tracer.name");
	  	}
	  }else{
	  	if(document.getElementById("SEL_xnat:petmrSessionData.tracer.name")!=undefined){
	  		appendImage("SEL_xnat:petmrSessionData.tracer.name","/images/checkmarkRed.gif");
	  	}else{
	  		appendImage("xnat:petmrSessionData.tracer.name","/images/checkmarkRed.gif");
	  	}
	   	  valid=false;
	  }
	  
	  if(valid){
		  var ih=getValueById("xnat:petmrSessionData/start_time_injection.hours");
		  if(ih!=null && ih!=undefined){
			var im=getValueById("xnat:petmrSessionData/start_time_injection.minutes");
			var is=getValueById("xnat:petmrSessionData/start_time_injection.seconds");
			var eh=getValueById("xnat:petmrSessionData/start_time_scan.minutes");
			var em=getValueById("xnat:petmrSessionData/start_time_scan.seconds");
		  	var es=getValueById("xnat:petmrSessionData/start_time_scan.seconds");
		  	if(ih.value!="" && ih.value!="bad"){
		  		if(im.value!="" && im.value!="bad"){
				    removeAppendImage("xnat:petmrSessionData/start_time_injection.minutes");
				}else{
				   	appendImage("xnat:petmrSessionData/start_time_injection.minutes","/images/checkmarkRed.gif");
				   	valid=false;
				}
		  	}
		  	...
		  	if(eh.value!="" && eh.value!="bad"){
		  		if(em.value!="" && em.value!="bad"){
				    removeAppendImage("xnat:petmrSessionData/start_time_scan.minutes");
				}else{
				   	appendImage("xnat:petmrSessionData/start_time_scan.minutes","/images/checkmarkRed.gif");
				   	valid=false;
				}
		  	}
