...
public class DicomSCPApi extends AbstractXapiRestController {
    ...
    @ResponseBody
    public ResponseEntity<DicomSCPInstance> dicomSCPCreate(@RequestBody final DicomSCPInstance instance) throws DuplicateEnabledDICOMReceiverPort {
        ...
        } catch (final NrgServiceException e) {
            switch (e.getServiceError()) {
                case AlreadyInitialized:
                    throw new DuplicateEnabledDICOMReceiverPort(e.getMessage());
                case Unknown:
    ...
    public ResponseEntity<DicomSCPInstance> dicomSCPInstanceCreateOrUpdate(@ApiParam(value = "The ID of the DICOM SCP receiver definition to create or update.", required = true) @PathVariable("id") final int id, @RequestBody final DicomSCPInstance instance) throws NotFoundException {
        HttpStatus status = isPermitted();
        if (status != null) {
            return new ResponseEntity<>(status);
        }
